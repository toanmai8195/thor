...
Service Go đọc event friend-service ghi vào Kafka, kiểm tra đúng contract rồi chuyển sang topic `friend_events` cho StarRocks; event sai vào `friend_events_dlq`. Thiết kế: [README gốc, mục 11](../../../README.md#11-event-gateway-go).
...
Gateway nằm trong `com/tm/infra/docker-compose.yml`. Cách chạy toàn bộ stack xem [`com/tm/friend-service/README.md`](../friend-service/README.md).
...
Ghi thẳng vào topic đầu vào, dạng `key|value` (chạy trong `com/tm/infra`):
...
```bash
cd com/tm/infra && docker compose up -d kafka && docker compose stop event-gateway && cd -
bazel run //com/tm/event-gateway:event_gateway      # mặc định Kafka localhost:29092, health ở cổng 8080
```
