...
> Đã hiện thực: `friend-service/` sinh event (mục 10), `event-gateway/` đọc lại, kiểm tra contract rồi chuyển sang `friend_events` (mục 11). Topic `friend_events` chỉ chứa event đúng contract.
...
> Bản đầy đủ (DDL, Routine Load, task, query): `friend_network.sql`. Bản local: `infra/starrocks/`.
...
> Chạy local: `infra/docker-compose.yml` có sẵn StarRocks, tự tạo bảng + Routine Load đọc `friend_events` (bản local của DDL: `replication_num = 1`, ít bucket, broker `kafka:9092`, task summary mỗi 1 phút, task daily mỗi 5 phút). Hướng dẫn: [`friend-service/README.md`](friend-service/README.md#6-xem-dữ-liệu-trên-starrocks).
...
Hiện có (`com/tm/server` là Bazel module root; web app ở `com/tm/app`, ngoài Bazel):

```
com/tm/server/
├── README.md  PHONEBOOK.md
├── MODULE.bazel  .bazelrc  .bazelversion  BUILD.bazel   # Bazel 8.7 (Go, Node.js, OCI)
├── go.mod                        # module thor/server
├── friend_network.sql            # toàn bộ DDL, Routine Load, task, query Q1–Q10
├── phonebook.sql                 # DDL, Routine Load, task, query của phonebook
...
├── phonebook-service/            # Go + HBase: danh bạ theo bucket (PHONEBOOK.md, README riêng)
│   ├── main.go  app.go  http_server.go                   # MainServer: DI (fx) + lifecycle
│   └── internal/{router,handler,controller,dao,utils,configs}/   # mỗi layer 1 BUILD.bazel
└── infra/
    ├── docker-compose.yml        # MongoDB, Kafka, StarRocks, HBase, event-gateway, friend-service, phonebook-service
...
Service nhận hành động của user qua REST API, lưu trạng thái trong MongoDB và gửi event (contract mục 2) lên Kafka topic `friend_service_events`; event-gateway (mục 11) kiểm tra rồi chuyển sang `friend_events`. Code ở `friend-service/`, Docker ở `infra/`.
...
Mỗi layer là 1 package Bazel (`ts_layer` = `ts_project` với tsconfig chung, `friend-service/ts.bzl`). `visibility` của Bazel chặn phụ thuộc sai chiều: vd Controller khai báo dep vào Router sẽ báo lỗi `is not visible` lúc build.
...
Luật nằm trong hàm thuần `plan()` ở `friend-service/src/controller/friendship-rules.ts`, có unit test cho từng ô của bảng.
...
> Hướng dẫn từng bước chạy thử trên Docker: [`friend-service/README.md`](friend-service/README.md).

Service build bằng macro `com_tm_js_image` (`tools/rules/com_tm_container.bzl`), sinh các target:

| Target | Việc |
|---|---|
| `//friend-service/src:friend_service` | `js_binary` chạy local |
| `//friend-service/src:mainserver` | `ts_project` của MainServer (`server.ts`, `container.ts`, `http-server.ts`) |
| `//friend-service/src/<layer>` | `ts_project` của từng layer (biên dịch + typecheck) |
| `//friend-service/src:friend_service_image` | `oci_image`: Node toolchain + node_modules + app |
| `//friend-service/src:friend_service_docker` | load image `com.tm.js.friend_service:v1.0.0` vào Docker |
| `//friend-service/test:<tên>_test` | `js_test` cho mỗi `test/*.test.ts`: `container`, `controller`, `event_publisher`, `friendship_rules`, `params`, `utils` |
...
bazel run --config=linux-arm64 //friend-service/src:friend_service_docker   # Mac Apple Silicon
bazel run --config=linux-amd64 //friend-service/src:friend_service_docker   # server x86
```
...
Thêm / đổi npm package: sửa `package.json` → `pnpm install` (cập nhật `pnpm-lock.yaml`) → thêm `//friend-service:node_modules/<package>` vào `deps` của layer dùng nó (vd `src/dao/BUILD.bazel`); package không kèm type thì thêm cả `@types/<package>`. Phiên bản TypeScript của Bazel lấy từ `devDependencies.typescript` (phải là phiên bản cụ thể). File `.ts` mới trong 1 layer tự vào build (`glob`). Package có build script phải khai báo trong `pnpm.onlyBuiltDependencies` của `package.json`.

Dev không qua Bazel (trong `friend-service`): `pnpm dev` (tsx watch), `pnpm test`, `pnpm typecheck`, `pnpm build && pnpm start` (ra `dist/`).

---

## 11. Event gateway (Go)

Service Go đứng giữa friend-service và StarRocks: đọc event friend-service ghi vào Kafka, kiểm tra đúng contract mục 2.3, chuyển event hợp lệ sang topic `friend_events` cho StarRocks Routine Load, event sai sang topic dead letter. Code ở `event-gateway/`, hướng dẫn chạy ở [`event-gateway/README.md`](event-gateway/README.md).
...
bazel run //event-gateway:event_gateway                                   # chạy local
bazel run --config=linux-arm64 //event-gateway:event_gateway_docker       # image com.tm.go.event_gateway:v1.0.0
