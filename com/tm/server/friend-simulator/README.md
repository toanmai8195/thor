...
- Giữ trạng thái các cặp user trong bộ nhớ và chỉ chọn hành động hợp lệ theo luật của friend-service (README server 10.5), nên phần lớn request thành công.
...
Mọi lệnh `bazel` chạy từ `com/tm/server` (Bazel module root). Stack (friend-service, Kafka, StarRocks...) chạy theo [`friend-service/README.md`](../friend-service/README.md).
...
Chạy trên máy (không qua container): `bazel run //friend-simulator:friend_simulator` (mặc định gọi `http://localhost:3000`).
...
Đổi trong `infra/docker-compose.yml` (service `friend-simulator`) rồi `docker compose --profile simulator up -d friend-simulator`.
